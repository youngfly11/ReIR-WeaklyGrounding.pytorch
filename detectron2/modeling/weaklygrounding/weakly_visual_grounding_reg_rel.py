...
from detectron2.modeling.weaklygrounding.loss import WeaklyVGLossComputeV3Cates
from detectron2.modeling.weaklygrounding.phrase_embedding_weakly_v1 import PhraseEmbeddingSentCate
...
from detectron2.modeling.weaklygrounding.PhraseGraph import WordPhraseGraphV1
from detectron2.layers.generate_union_region import generate_union_region_boxes
from detectron2.modeling.weaklygrounding.StructureVisualGraph import StructureGraphMessagePassing
...
class DetProposalVGHead(torch.nn.Module):
    def __init__(self, roi_pooler, RCNN_top):
        ...
        self.phrase_embed = PhraseEmbeddingSentCate(cfg, phrase_embed_dim=self.phrase_embed_dim, bidirectional=cfg.MODEL.VG.LSTM_BIDIRECTION)
        ...
        if cfg.MODEL.RELATION.IS_ON:
            self.relation_merge = nn.Sequential(
                nn.Linear(2 * self.visual_embed_dim, self.visual_embed_dim),
                nn.ReLU(inplace=True))

            self.rel_trans = nn.Linear(self.visual_embed_dim, 512)
            self.agg_trans = nn.Sequential(nn.Linear(self.visual_embed_dim, self.visual_embed_dim), nn.ReLU(inplace=True))
            self.vis2rel = nn.Linear(self.visual_embed_dim, self.phrase_embed.rel_cater_size)

        self.vis2phr = nn.Linear(self.rnn_hidden_dim, self.phrase_embed.phr_vocab_size)
        self.VGLoss = WeaklyVGLossComputeV3Cates()
    ...
    def forward(self, features, all_phrase_ids, targets, precomp_boxes, precomp_score,
        ...
        for bid in range(img_num_per_gpu):
            ...
            if cfg.MODEL.RELATION.IS_ON:
                ...
                if len(relation_conn_bid) > 0:
                    ...
                    phr_sub_idx, phr_obj_idx = torch.as_tensor(relation_conn_bids).to(self.device).long().transpose(0, 1)
                    visual_ctx_topk = (atten_score_topk.unsqueeze(2) * visual_features_topk_bid.reshape(num_phrase, self.s2_topk, -1)).sum(1) ## N*1024
                    visual_trans = self.rel_trans(visual_ctx_topk)
                    ent_gate = (visual_trans[phr_sub_idx] * visual_trans[phr_obj_idx]).sum(1) / 512**0.5
                    gate_mat = torch.zeros([num_phrase, num_phrase]).to(self.device)
                    ...
                    gate_mat = numerical_stability_masked_softmax(gate_mat, gate_mask, dim=1)
                    # cxt_feat = torch.mm(gate_mat, visual_ctx_topk)
                    cxt_feat = self.agg_trans(torch.mm(gate_mat, visual_ctx_topk))
                    visual_features_topk_bid = visual_features_topk_bid + torch.bmm(atten_score_topk.unsqueeze(2), cxt_feat.unsqueeze(1)).view(-1, self.visual_embed_dim)
            ...
            if cfg.MODEL.RELATION.IS_ON:
                ...
                if len(relation_conn_bid) > 0:
                    ent_idx = []
                    rel_cates = torch.zeros([len(relation_conn_bid), self.phrase_embed.rel_cater_size]).to(self.device).float()
                    for rel_i, conn in enumerate(relation_conn_bid):
                        phr_sub_id, phr_obj_id, rel_cate, rel_id = conn
                        rel_cates[rel_i][rel_cate] = 1   ## indicate the target relation
                        ent_idx.append([phr_sub_id, phr_obj_id])

                    phr_sub_ids, phr_obj_ids = torch.as_tensor(ent_idx).long().to(self.device).transpose(0,1)
                    visual_rel_feats_s0 = torch.cat((visual_reconst_bid[phr_sub_ids], visual_reconst_bid[phr_obj_ids]), dim=1)
                    visual_rel_feats_s0 = self.relation_merge(visual_rel_feats_s0)
                    rel_logits_s0 = self.vis2rel(visual_rel_feats_s0)
                    batch_rel_cls_s0.append(rel_logits_s0)
                    # batch_rel_cls_s0.append(None)

                    visual_rel_feats_s1 = torch.cat((visual_reconst_topk_bid[phr_sub_ids], visual_reconst_topk_bid[phr_obj_ids]), dim=1)
                    visual_rel_feats_s1 = self.relation_merge(visual_rel_feats_s1)
                    rel_logits_s1 = self.vis2rel(visual_rel_feats_s1)
                    batch_rel_cls_s1.append(rel_logits_s1)
                    batch_rel_cls_gt.append(rel_cates) ## change the ground-truth into cuda tensor
        ...
        all_loss = dict(noun_reconst_loss=noun_reconst_loss, noun_topk_reconst_loss=noun_topk_reconst_loss, disc_img_sent_loss_s1=disc_img_sent_loss_s1,
                        disc_img_sent_loss_s2=disc_img_sent_loss_s2, reg_loss_s1=reg_loss, reg_loss_s2=reg_loss_s1, rel_cls_loss=rel_cls_loss, rel_cls_loss_s1=rel_cls_loss_s1,
                        rel_const_loss=rel_const_loss, rel_const_loss_s1=rel_const_loss_s1)
    ...
    def rel_phrase_decoder(self, visual_reconst, rel_enc_embed):
        ...
        enc_embed = torch.cat((visual_reconst.unsqueeze(1), rel_enc_embed), dim=1) ## bs*maxl*L
        dec_emb, _ = self.dec_rel_gru(enc_embed)
        decode_rel_logits = self.visual2rel_reconst(dec_emb)
        # dec_emb, _ = self.dec_phrase_gru(enc_embed)
