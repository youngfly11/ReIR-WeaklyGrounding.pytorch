...
class Boxes:
    ...
    def __init__(self, tensor: torch.Tensor, box_size: list):
        """
        Args:
    ...
    def clone(self) -> "Boxes":
        ...
        Returns:
            Boxes
        """
        return Boxes(self.tensor.clone(), self.size)

    def to(self, device: str) -> "Boxes":
        return Boxes(self.tensor.to(device), self.size)
    ...
    def __getitem__(self, item: Union[int, slice, torch.BoolTensor]) -> "Boxes":
        ...
        if isinstance(item, int):
            return Boxes(self.tensor[item].view(1, -1), self.size)
        b = self.tensor[item]
        assert b.dim() == 2, "Indexing on Boxes with {} failed to return a matrix!".format(item)
        return Boxes(b, self.size)
...
def phrase_boxlist_union(boxlist1:Boxes, boxlist2:Boxes, phrsbj2obj_scores=None, phrsbj2obj_scores_sort_id=None, cluster=True):
    """
    ...
    """
    # build phrase region
    box1, box2 = boxlist1.tensor, boxlist2.tensor
    lt = torch.min(box1[:, :2], box2[:, :2])  # [N,M,2]
    rb = torch.max(box1[:, 2:], box2[:, 2:])  # [N,M,2]
    phrase_proposal = torch.cat((lt, rb), dim=1)
    phrase_proposal = Boxes(phrase_proposal, boxlist1.size)
    phrase_proposal = phrase_proposal[phrsbj2obj_scores_sort_id]

    # cluster the phrase region
    if cluster:
        phrase_proposal_nms, keep = boxlist_nms(phrase_proposal, 0.95, scores=phrsbj2obj_scores, require_keep_idx=True)
        # retrieval the reduced boxes are clustered by which preserved box
        proposal_cluster = pairwise_iou(phrase_proposal, phrase_proposal_nms)
        _, cluster_idx = torch.max(proposal_cluster, dim=1)
    else:
        phrase_proposal_nms = phrase_proposal
        cluster_idx = torch.arange(len(phrase_proposal), dtype=torch.int64, device=box1.device)
        keep = cluster_idx

    return phrase_proposal, phrase_proposal_nms, cluster_idx, keep


def boxlist_nms(boxlist, nms_thresh, max_proposals=-1, scores=None, require_keep_idx=False):
    """
...
def boxlist_nms_tensor(boxlist, nms_thresh, max_proposals=-1, scores=None, require_keep_idx=False):
    """
    Performs non-maximum suppression on a boxlist, with scores specified
    in a boxlist field via score_field.
    Arguments:
