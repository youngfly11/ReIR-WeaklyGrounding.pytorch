...
class FLICKR30KEvaluator(DatasetEvaluator):
    ...
    def process(self, inputs, outputs):
        ...
        all_topk_sim = outputs[7]


        for phrase_ids, phrase_type, pred_boxes, pred_sim, targets, precomp_boxes, img_sent_id, topk_sim in zip(all_phrase_ids, all_phrase_type, all_pred_boxes, all_pred_similarity, all_targets, all_precomp_boxes, all_img_sent_ids, all_topk_sim):
            ...
            # topk_sim = topk_sim.to(self._cpu_device)

            self._predictions.update({img_sent_id: [phrase_ids, phrase_type, pred_boxes, pred_sim, targets, precomp_boxes, topk_sim]})
    ...
    def evaluate(self):
        ...
        del all_prediction
        self._logger.info('evaluation on {} expression instances, detailed_iou: {}'.format(num_phrs, acc_type))
