...
class FLICKR30KEvaluator(DatasetEvaluator):
    ...
    def process(self, inputs, outputs):
        ...
        # move2cpu(batch_topk_precomp_boxes), move2cpu(batch_topk_pred_targets), move2cpu(batch_pred_targets), move2cpu(
        #     batch_topk_s2_pred_boxes)
        ...
        all_topk_s2_pred_boxes = outputs[14]
        ...
        for phrase_ids, phrase_type, pred_boxes, pred_sim, targets, precomp_boxes, img_sent_id, topk_pred_sim, topk_fusion_pred_sim, topk_pred_box, topk_fusion_pred_boxes, topk_precomp_boxes,\
            topk_pred_targets, pred_targets, topk_s2_pred_box in zip(all_phrase_ids, all_phrase_type, all_pred_boxes, all_pred_similarity, all_targets, all_precomp_boxes,
                       ...
                       all_topk_pred_targets, all_pred_targets, all_topk_s2_pred_boxes):
            ...
            self._predictions.update({img_sent_id: [phrase_ids, phrase_type, pred_boxes, pred_sim, targets, precomp_boxes, topk_pred_box, topk_pred_sim, topk_fusion_pred_boxes, topk_fusion_pred_sim,
                                                    topk_precomp_boxes, topk_pred_targets, pred_targets, topk_s2_pred_box]})
    ...
    def evaluate(self):
        ...
        for img_sent_id in image_unique_ids:
            ...
            topk_s2_pred_boxes.clip()
            ious_topk_s2 = pairwise_iou(targets, topk_s2_pred_boxes)
            recall_t3_num += int((ious_topk_s2.numpy().diagonal()>=cfg.MODEL.VG.EVAL_THRESH).sum())
        ...
        del all_prediction
        self._logger.info('Evaluate Multi-Stage Reg: evaluation on {} expression instances, detailed_acc: {}'.format(len(image_unique_ids), acc_type))
        results = OrderedDict({"acc": acc, "acc_top5": acc_top5, "acc_top10": acc_top10, 'acc_s2': acc_s2, 'acc_s2_fusion': acc_s2_fusion, 'acc_s3': acc_s3})
        return results