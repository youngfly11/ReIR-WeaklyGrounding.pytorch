...
from .common import DatasetFromList, MapDataset, AspectRatioGroupedDataset
...
def make_batch_data_sampler(
        dataset, sampler, aspect_grouping, aspect_ratios, images_per_batch, num_iters=None, start_iter=0
):
    if aspect_grouping:
...
def get_flickr30k_dataset_dicts(dataset_names):
...
def build_detection_train_loader(cfg, mapper=None):
    ...
    dataset_dicts = get_flickr30k_dataset_dicts(cfg.DATASETS.TRAIN)

    logger = logging.getLogger(__name__)
    logger.info(
        "There are {} expression instance in training set.".format(len(dataset_dicts))
    )

    # DatasetFromList is the traditional dataloader, which contain __get_item__ and __len__ function.
    # but in this function we just pass some light-weight annotation and image path.
    ...
    batch_collator = trivial_batch_collator
...
def build_detection_test_loader(cfg, dataset_name, mapper=None):
    ...
    """
    dataset_dicts = get_flickr30k_dataset_dicts([dataset_name])
...
def trivial_batch_collator(batch):
    ...
    sentence_list = []
    precomp_bbox_list = []
    precomp_det_label_list = []
    det_label_embedding = []
    precomp_score_list = []
    image_scale_list = []
    image_unique_id_list = []


    for batch_sample in batch:

        image_list.append(batch_sample['image'])
        phrase_ids_list.append(batch_sample['phrase_ids'])
        gt_boxes_list.append(batch_sample['gt_boxes'])
        precomp_bbox_list.append(batch_sample['precomp_bbox'])
        precomp_score_list.append(batch_sample['precomp_score'])
        precomp_det_label_list.append(batch_sample['precomp_det_label'])
        det_label_embedding.append(batch_sample['det_label_embedding'])
        image_scale_list.append(batch_sample['image_scale'])

        relations_list.append(batch_sample['relations'])
        sentence_list.append(batch_sample['sentence'])

        image_unique_id_list.append(batch_sample['image_id'] + "_" + batch_sample['sent_id'])

    del batch
    return image_list, phrase_ids_list, gt_boxes_list, precomp_bbox_list, precomp_score_list, precomp_det_label_list,\
           image_scale_list, relations_list, sentence_list, image_unique_id_list, det_label_embedding





def trivial_batch_collator_refcoco(batch):
    """
    A batch collator that does nothing.
    """
    image_list = []
    ...
    for batch_sample in batch:
        ...
        sentence_list.append(batch_sample['sentence'])
        image_unique_id_list.append(batch_sample['image_id'] + "_" + batch_sample['sent_id'])
        sent_noun_list.append(batch_sample['nouns'])
        sent_noun_phr_list.append(batch_sample['noun_phr'])
        sent_attr_lab_list.append(batch_sample['attr_lab'])

    del batch
    return image_list, gt_boxes_list, precomp_bbox_list, precomp_score_list, precomp_det_label_list,\
           image_scale_list, sentence_list, sent_noun_list, sent_noun_phr_list, image_unique_id_list, det_label_embedding, sent_attr_lab_list
